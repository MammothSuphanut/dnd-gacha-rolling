...
8. ใช้ภาษาไทยตลอดบทสนทนาและในไฟล์ที่สร้าง (ชื่อเฉพาะ class/subclass/feat/item คงเป็นภาษาอังกฤษตามต้นฉบับ)

## หัวข้อที่ต้องถามให้ครบ (ตามลำดับ)

1. **ชื่อตัวละคร** (ใช้ตั้งชื่อโฟลเดอร์ด้วย)
2. **Edition**: 2014 (PHB) หรือ 2024 (XPHB) — ยึดเป็นหลักตลอดการ build นี้ (ถ้าใช้ homebrew ที่มีสองเวอร์ชัน ให้เลือกไฟล์ตรง edition นี้)
...
4. **Level เริ่มต้น**: level 1 ตามปกติ หรือเริ่มที่ level อื่น (ถ้าเข้าปาร์ตี้ที่เล่นไปแล้ว)
5. **Class**: เสนอตัวเลือกที่เข้ากับคอนเซปต์ทั้งจาก official และ homebrew พร้อมแหล่งอ้างอิง อธิบายจุดเด่นสั้นๆ ต่อคลาส
6. **Subclass**: ถ้า level เริ่มต้น ≥ level ที่ class นั้นได้ subclass (เช็คจากข้อมูล class จริง เช่น Cleric/Sorcerer ได้ตอน level 1, Wizard/Druid ตอน level 2, ส่วนใหญ่ตอน level 3) ให้ถามเลือก subclass เลย ถ้ายังไม่ถึง level ให้บันทึกไว้ใน Build Notes ว่ายังไม่ได้เลือก
