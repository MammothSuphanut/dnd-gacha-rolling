...
### กฎ 2.5 — ตอนย่อย (side episode) ที่เล่นนอกเนื้อเรื่องหลัก

บางเซสชันผู้เล่นตัดสินใจไปทำอย่างอื่นนอกแผนเดิมทั้งเซสชัน (ไปเที่ยว, ปลีกตัวไปทำธุระส่วนตัวของตัวละคร, เควสต์ข้างทางที่ไม่ได้เตรียมไว้ล่วงหน้า) — เนื้อหานั้นยังต้องมีที่บันทึกเป็น journal เต็มเหมือน EP ปกติ แต่**ไม่นับเป็นเนื้อเรื่องหลัก**

- ตั้งชื่อไฟล์โดย**ต่อท้ายเลข EP หลักที่มันแทรกอยู่หลังด้วยตัวอักษรพิมพ์เล็ก** แทนการเลื่อนเลข EP ทุกตัวที่ตามมา — รูปแบบ: `EP0Y` + ตัวอักษร (`a`, `b`, `c`, ...) + ` - ชื่อตอนเต็ม.md` เช่น เล่นถึง `EP04 - Travel to Syrinlya.md` แล้วออกนอกเรื่องก่อนเข้า EP05 เดิม → ไฟล์ใหม่ชื่อ `EP04a - The Smuggler's Debt.md` (ถ้ามีตอนย่อยที่สองต่อจาก EP04 อีก ใช้ `EP04b - ...md`) — **ห้ามเปลี่ยนเลขไฟล์ `EP05` เดิมเด็ดขาด** เหตุผลที่ใช้ตัวอักษรต่อท้ายแทนการนับเลขใหม่ทั้งหมด: ตอนย่อยมักถูกเพิ่มย้อนหลังหลังจากเล่นจบเซสชันไปแล้ว การเลื่อนเลข EP ทุกไฟล์หลังจากนั้นจะทำให้ลิงก์อ้างอิงเดิม (ใน `00-act-outline.md`, ไฟล์ EP อื่นๆ, หรือที่แชร์ไปแล้ว) พังหมด
- หัวเรื่อง `#` บนสุดของไฟล์ใช้รูปแบบเดียวกับ EP ปกติ (`# EP4a: The Smuggler's Debt`) และเขียนตามกฎข้อ 1-11 ของ `journal-template.md` ทุกอย่างเหมือน EP ปกติ (ตาราง NPC, Story Overview, tactics, treasure ฯลฯ)
- ไม่ต้องเพิ่มลิสต์ตอนย่อยไว้ใน `00-act-outline.md` (มันไม่ได้อยู่ในลำดับเนื้อเรื่องหลักที่ outline วางแผนไว้) — แค่ลิงก์ไป-กลับระหว่างไฟล์ EP หลักที่มันแทรกอยู่หลัง กับ EP หลักถัดไปที่กลับเข้าเนื้อเรื่องเดิม ก็พอ (เช่นท้าย `EP04 - Travel to Syrinlya.md` เพิ่มลิงก์ "ถ้าตอนจบเซสชันนี้กลุ่มออกนอกเรื่อง ดู [EP04a](<./EP04a - The Smuggler's Debt.md>)" และท้าย `EP04a` ลิงก์กลับเข้า `EP05` ตามปกติ)
- แอปฝั่ง UI จะรู้จักไฟล์ที่มีตัวอักษรต่อท้ายเลขอัตโนมัติ (ดู `src/utils/adventureJournals.js`) และแสดงเป็น "ตอนย่อย" แยกจาก EP หลักในแถบเมนู Journal — ไม่ต้องทำอะไรเพิ่มฝั่งแอป

**ข้อยกเว้น — หลายโต๊ะเล่นเรื่องเดียวกันคู่ขนาน**: ถ้า adventure เดียวกันถูกเล่นโดยมากกว่า 1 โต๊ะ/กลุ่มผู้เล่นพร้อมกัน (เช่นโต๊ะ "ats" กับโต๊ะ "w") และแต่ละโต๊ะแยกออกไปทำ side episode ของตัวเองหลัง EP หลักเดียวกัน ตัวอักษรเฉยๆ (`a`, `b`, `c`) จะชนกันไม่รู้ว่าไฟล์ไหนเป็นของโต๊ะไหน — ในกรณีนี้ให้ตั้งชื่อเป็น `EP0Y` + **แท็กโต๊ะ** (lowercase, ตามที่โต๊ะนั้นเรียกตัวเอง) + **เลขลำดับ side session ของโต๊ะนั้น** (นับแยกต่างหากทีละโต๊ะ เริ่มที่ 1) แทนตัวอักษร a/b/c เช่น `EP07ats1 - Wolf Hunt on the Snowfields.md` (side session แรกของโต๊ะ ats หลัง EP07), ถ้าโต๊ะ ats มี side session ที่สองต่อจาก EP07 อีกก็เป็น `EP07ats2 - ...md`, ส่วนโต๊ะ w ถ้าทำ side session ของตัวเองหลัง EP07 เช่นกันก็เป็น `EP07w1 - ...md` แยกไฟล์กันชัดเจน ไม่ชนกัน — หัวเรื่อง `#` บนสุดของไฟล์ก็ใช้แท็กเดียวกัน (`# EP7ats1: Wolf Hunt on the Snowfields`) กฎอื่นๆ (ตาราง NPC, tactics, treasure ฯลฯ) เหมือนตอนย่อยปกติทุกประการ

### กฎ 2.6 — EP แยกสาขา (branching path) เมื่อเนื้อเรื่องต้นฉบับแตกแขนงตามตัวเลือกที่ยังไม่ fix

บาง adventure (โดยเฉพาะ official module ที่แปล) ออกแบบให้ตอนหนึ่งแตกเป็นหลายเวอร์ชันคู่ขนานตามตัวแปรที่ DM เลือกไว้ล่วงหน้า (เช่น "วายร้ายหลักที่เลือก" ใน `gilded-city-mad-depths` ที่ Chapter 4 ของ Dragon Heist แตกเป็น 4 สาย: Spring=Xanathar / Summer=Cassalanters / Autumn=Jarlaxle / Winter=Manshoon — เล่นจริงแค่สายเดียว) ถ้ายังไม่อยาก fix ตัวแปรนั้นตอนแปล ให้เขียนทุกสายเป็น journal เต็มแยกไฟล์กัน แล้วปล่อยให้ DM เลือกไฟล์ที่ตรงกับตัวเลือกจริงตอนเล่น แทนที่จะบังคับเลือกล่วงหน้าตอนเตรียมเอกสาร

- **แต่ละสาขาได้เลข EP ของตัวเอง เรียงต่อเนื่องกันไปเลย ไม่ใช้เลขซ้ำกัน** (ต่างจากตอนย่อยในกฎ 2.5 ที่ใช้ตัวอักษรต่อท้ายเลขเดิม) เหตุผล: แอปฝั่ง UI (`src/utils/adventureJournals.js`) sort/key EP ด้วยเลขที่ไม่ซ้ำกันเป็นหลัก การใช้เลขเดียวกันซ้ำ 4 ไฟล์จะชน id กัน — เช่น Ch4 ของ Dragon Heist แตก 4 สาย ก็ได้ `EP04` ถึง `EP07` (คนละไฟล์คนละสาย) ไม่ใช่ `EP04` ซ้ำ 4 หน
- ชื่อไฟล์ต้องระบุชื่อสายให้ชัดในหัวเรื่องเต็ม เพื่อไม่ให้สับสนว่าไฟล์ไหนคือสายไหน เช่น `EP04 - Spring Encounter Chain (Xanathar).md`, `EP05 - Summer Encounter Chain (Cassalanters).md`
- ถ้าทุกสายมาบรรจบจุดจบร่วมกัน (เช่น dungeon สุดท้ายเดียวกันไม่ว่าจะมาทางไหน) ให้แยกจุดบรรจบนั้นเป็น EP ของตัวเองอีกไฟล์ (เลขถัดจากสาขาสุดท้าย) แทนการก็อปเนื้อหาเดียวกันซ้ำ 4 รอบในแต่ละสาขา — ทุกสาขาลิงก์ไปจบที่ไฟล์เดียวกันนี้
- `00-act-outline.md`/`CH0X-slug.md` ต้องระบุชัดว่าเลข EP ไหนเป็น "ทางเลือก" ของกันและกัน (เล่นแค่ 1 ใน N ไฟล์นั้นตามตัวเลือกที่เลือกจริง) ไม่ใช่เล่นต่อกันทั้งหมด — กันสับสนว่าทำไมเลข EP ไม่ต่อเนื่องเป็นเนื้อเรื่องเดียว
- ตัวอย่างจริง: ดู `adventures/gilded-city-mad-depths/ACT01-dragon-heist/00-act-outline.md`
