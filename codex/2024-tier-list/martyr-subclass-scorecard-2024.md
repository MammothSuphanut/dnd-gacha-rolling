...
| **Survivability** | 10/10 (S) | Fade from View (ล่องหนตอน initiative ฟรี) + False Martyrdom (lv14, ปลอมตายทีเลพอร์ตหนีพร้อมศพปลอม) — ชั้นการหลบหลีกที่ทับซ้อนบน baseline ที่แข็งอยู่แล้ว [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 7/10 (A) | Divine Assassin (lv18) แคส Invisibility/Greater Invisibility เป็น bonus action ได้ + เสีย Radiant damage ครึ่งเดียว [M:2/4] [R:3/3] [B:2/3] |
| **Utility** | 8/10 (A) | Passwall/Modify Memory/Sending ติดตัวถาวร + Thieves' Tools proficiency — utility แทรกซึม/สายลับที่ baseline ไม่มี [M:2/4] [R:3/3] [B:3/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 8/10 (A) | Herculean Surge (บวก Str/Dex ชั่วคราวเสริมโจมตี) + Xiphos relic (lv14, ตีเพิ่มฟรีทุกครั้งที่ครี/ฆ่า) [M:2/4] [R:3/3] [B:3/3] |
| **Control** | 5/10 (B) | Hold Monster ติดตัวถาวรตั้งแต่ lv17 เป็น hard-CC ที่ baseline ไม่มี [M:2/4] [R:1/3] [B:2/3] |
| **Support** | 3/10 (C) | ไม่มี feature เสริมทีมใหม่ = เท่า baseline |
| **Survivability** | 10/10 (S) | Legendary Resistance (lv18, เปลี่ยน failed save เป็น success 1/long rest) + Aegis relic (แคส Shield ฟรีไม่เสีย Radiant dmg) [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 7/10 (A) | Xiphos relic ให้ตีเพิ่มฟรีไม่เสีย action หลังครี/ฆ่า [M:2/4] [R:3/3] [B:2/3] |
| **Utility** | 8/10 (A) | Olympic Agility (Climb+Swim Speed เต็ม + เพิกเฉย difficult terrain) + Oracle relic (แคส Commune ฟรี) [M:2/4] [R:3/3] [B:3/3] |
...
| **Utility** | 8/10 (A) | Sin Eater ใช้ล้างคำสาป/Exhaustion/ability drain ให้ ally ได้เหมือน Remove Curse ระดับสูง [M:2/4] [R:3/3] [B:3/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 9/10 (S) | Aura of Calamity (action, AoE emanation ต่อเนื่อง 1 นาที สเกล 1d8→4d8 ธาตุที่เลือก+Prone) + Doom Foretold (lv18, แคส Earthquake/Fire Storm/Storm of Vengeance ฟรี) — AoE nova ที่หนักที่สุดในกลุ่ม [M:3/4] [R:3/3] [B:3/3] |
...
| **Utility** | 7/10 (A) | Apocalyptic Omen (lv6, บวก 1d4 เข้า d20 test ทุกแบบนาน 1 ชม.) เป็นบัฟสากลที่ baseline ไม่มี [M:2/4] [R:3/3] [B:2/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 9/10 (S) | Havoc! table (d10, ตีเมลีโดนแล้วทอย) มีตัวเลือกดาเมจเสริม+auto-crit บวก Sacrificial Burst (lv18, อัปเกรด Sacrificial Strike เป็น exploding dice สูงสุด 10 ก้อน) — magnitude สูงสุดในกลุ่ม [M:4/4] [R:2/3] [B:3/3] |
...
| **Action Economy** | 7/10 (A) | Pandemonium (lv14) ตัด Radiant cost ของ Havoc! ออก + ทอยสองครั้งเลือกเอา — resource efficiency ที่ดีขึ้น [M:2/4] [R:3/3] [B:2/3] |
| **Utility** | 7/10 (A) | Coin of Chaos (lv6, บวก +4/+1 เข้า d20 test ใดก็ได้ แชร์เหรียญกับ GM) เป็นบัฟสากลที่ baseline ไม่มี [M:2/4] [R:3/3] [B:2/3] |
...
| **Survivability** | 10/10 (S) | Entourage (lv6, บวก AC เท่าจำนวนเป้า Charmed รอบตัว สูงสุดเท่า Wis mod) + Reactive Sanctuary (lv18, แคส Sanctuary ฟรีพร้อม Reprisal) [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 6/10 (B) | ไม่มี economy gain ใหม่ที่ชัดเจน = เท่า baseline |
| **Utility** | 7/10 (A) | Tongues/Teleportation Circle/Private Sanctum ติดตัวถาวร + Celebrity (Performance boost) [M:2/4] [R:3/3] [B:2/3] |
...
| **Support** | 3/10 (C) | ไม่มี feature เสริมทีมใหม่ = เท่า baseline |
| **Survivability** | 10/10 (S) | Just a Prank (lv6, ตีสวนศัตรูที่จะทำ OA ใส่เราก่อนที่ OA จะลง — เท่ากับยกเลิก OA ได้บ่อยๆ) + Zany Misdirection (lv18, Reprisal เปลี่ยนเป้าการโจมตีไปโดนตัวอื่นแทนได้เลย) [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 7/10 (A) | Just a Prank ให้ตีฟรีก่อนโดน OA ไม่เสีย reaction/action เพิ่ม [M:2/4] [R:3/3] [B:2/3] |
| **Utility** | 7/10 (A) | Dimension Door/Modify Memory/Seeming ติดตัวถาวร [M:2/4] [R:3/3] [B:2/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 8/10 (A) | Censuring Light (lv14, AoE 4d6 radiant ต่อเนื่องใส่ creature type ที่เลือก) [M:2/4] [R:3/3] [B:3/3] |
...
| **Utility** | 7/10 (A) | Virtuous Blessings (Blessing of Alacrity/Eloquence/Vigilance) เป็น utility เสริมทีมที่กว้าง [M:2/4] [R:3/3] [B:2/3] |
...
| **Survivability** | 10/10 (S) | Zephyr's Stride (lv14, Disadvantage การโจมตีใส่เราหลังเคลื่อนที่ 30ft+) + Argonaut (lv18, Haste ฟรีตอน initiative ไม่มี drawback) [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 8/10 (A) | Swift Reprisal ให้ใช้ Reprisal ได้โดยไม่เสีย reaction เลยตอนเป็นเทิร์นตัวเอง [M:3/4] [R:2/3] [B:3/3] |
| **Utility** | 8/10 (A) | Freedom of Movement/Passwall/Teleportation Circle/Water Walk ติดตัวถาวร — exploration/travel utility ที่กว้างมาก [M:2/4] [R:3/3] [B:3/3] |
...
| **Survivability** | 10/10 (S) | Verdant Resilience (lv14, bonus action 1 นาที: ฟื้น HP ทุกเทิร์น + resistance B/P/S) + Turn of the Wheel (lv18, ฟื้นคืนชีพตัวเองได้ 1 ครั้งถ้ายังไม่ใช้ Final Martyrdom) [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 6/10 (B) | ไม่มี economy gain ใหม่ที่ชัดเจน = เท่า baseline |
| **Utility** | 8/10 (A) | Awaken/Reincarnate/Speak with Plants/Hallucinatory Terrain ติดตัวถาวร + familiar ฟรี — nature utility ที่กว้างมาก [M:2/4] [R:3/3] [B:3/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 8/10 (A) | Kingslayer (lv14) เลือก Assault ได้: bonus action เคลื่อนที่+ตีฟรีทันทีที่ฆ่าศัตรู [M:2/4] [R:3/3] [B:3/3] |
...
| **Utility** | 7/10 (A) | Telepathic Bond ติดตัวถาวร + Aura of Tenacity (initiative buff วงกว้าง) [M:2/4] [R:3/3] [B:2/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 8/10 (A) | เข้าถึง Chromatic Orb/Lightning Bolt/Cone of Cold + Sacrificial Spell's Quicken (lv14, แคส spell เป็น bonus action ได้ — เท่ากับแคสสองครั้ง/เทิร์น) [M:2/4] [R:3/3] [B:3/3] |
...
| **Utility** | 7/10 (A) | Counterspell/Telekinesis ติดตัวถาวร + Mystical Wisdom (Arcana boost) [M:2/4] [R:3/3] [B:2/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 8/10 (A) | เข้าถึง Word of Force/Word of Frailty (สเปลเฉพาะทาง VSS) ติดตัวถาวร [M:2/4] [R:3/3] [B:3/3] |
...
| **Survivability** | 10/10 (S) | Forewarning ใช้กับตัวเองได้เช่นกัน + Seer (lv6, Blindsight 30ft) [M:4/4] [R:3/3] [B:3/3] |
| **Action Economy** | 7/10 (A) | Sacrificial Foresight (lv18, บวก +5 เข้า d20 test **ทุกอัน**จนจบเทิร์นจาก Sacrificial Skill ครั้งเดียว) [M:2/4] [R:3/3] [B:2/3] |
| **Utility** | 8/10 (A) | Divination/Legend Lore/Sending/Freedom of Movement ติดตัวถาวร + Seer [M:2/4] [R:3/3] [B:3/3] |
...
| Axis | คะแนน | เหตุผล |
|---|---|---|
| **Damage** | 8/10 (A) | Totalitarian Control's Psychic Recoil (lv18, 6d8 psychic เมื่อ Dominate spell จบ) [M:2/4] [R:3/3] [B:3/3] |
...
| **Survivability** | 10/10 (S) | Sacrificial Resilience (lv14, ได้ temp HP เท่า Radiant damage ที่เสียจาก Sacrificial Strike — คืนทุนความเสียหายที่จ่ายไปทันที) [M:4/4] [R:3/3] [B:3/3] |
