...
**🔧 Refinement pass รอบ 9 (2026-08-14)**: cross-check spell text ที่เหลือใน Utility/Control Anchor ที่ยังไม่เคยตรวจละเอียด (Knock/Clairvoyance/Tongues/Water Breathing/Divination/Legend Lore/Scrying/Teleportation Circle/True Seeing/Teleport/Plane Shift/Mind Blank/Wish + Banishment/Hypnotic Pattern/Mass Suggestion) เจอ 3 จุด — (a) **Mind Blank (lv15, Utility) ขาด Psychic damage immunity** — spell text จริงมี 2 ส่วน (immune divination/charm-detection ครบ + immune ดาเมจ Psychic ด้วย) เดิมตารางเขียนถึงแค่ส่วนแรก เพิ่มส่วนที่สองแล้ว, (b) **Knock (lv3, Utility) overstate ว่า "เบ็ดเสร็จ" ทั้งที่จริงเปิดได้แค่ 1 ล็อกต่อ 1 cast ถ้าของมีหลายล็อก** พร้อมเกิดเสียงดังได้ยินไกล 300ft ทุกครั้งที่ cast (ทั้ง 2 จุดเคยละไว้) — เพิ่มแล้ว, (c) **Banishment (lv7, Control) ขาด clause ที่สำคัญที่สุดของสเปลล์นี้** — ถ้าเป้าเป็น Aberration/Celestial/Elemental/Fey/Fiend และ concentration อยู่ครบ 1 นาทีเต็ม เป้าจะ**ไม่กลับมาเลย** (ส่งไป plane ที่เกี่ยวข้องถาวร) เท่ากับ removal ถาวรสำหรับ creature type เหล่านี้ ไม่ใช่แค่ "หายชั่วคราว" — เดิมตารางไม่เคยพูดถึงจุดนี้เลย เพิ่มแล้ว เพราะกระทบการประเมิน Severity อย่างมีนัยสำคัญ — cross-check ที่เหลือ (Teleport mishap table, Mass Suggestion duration/break-condition, Hypnotic Pattern, Scrying, Legend Lore, True Seeing, Water Breathing, Teleportation Circle) ยืนยันตรงกับไฟล์จริงหมด ไม่ต้องแก้ — **ทุกจุดที่เจอรอบนี้เป็น "completeness gap" (รายละเอียดที่ขาดหาย) ไม่ใช่ "ตัวเลขผิด" เหมือนรอบ 8 — สะท้อนว่าบั๊กเชิงตัวเลข/กติกาหลักน่าจะหมดแล้ว เหลือแต่รายละเอียดรองที่ยังไม่ได้ enrich ครบทุกจุด**

**🔧 Refinement pass รอบ 10 (2026-08-14)**: user ถามว่า "ทำไมยังเจอเรื่อยๆ" — cross-check duration/concentration field เพิ่มเติมใน Support (Beacon of Hope, Death Ward, Freedom of Movement, Holy Aura, Aid, Regenerate, Bless) เจอ (a) **Beacon of Hope (lv5) เขียนเกินจริงว่า "กัน unconscious"** ทั้งที่ spell text จริงคือ advantage WIS save + advantage Death Saving Throw + max healing เท่านั้น ไม่ใช่ immunity ต่อการสลบตรงๆ — แก้คำอธิบายแล้ว, (b) **พบว่า Death Ward/Freedom of Movement (lv7) ไม่ใช่ concentration spell เลย** (ต่างจาก Beacon/Haste/Holy Aura ที่เป็น concentration ทั้งหมด) ซึ่งเป็นข้อได้เปรียบเชิงโครงสร้างจริงที่ไม่เคยถูกพูดถึงมาก่อน — **นำไปสู่การพบช่องว่างเชิงโครงสร้างใหญ่กว่า**: ทั้ง Control และ Support axis ไม่มีมิติไหนวัด "caster concentration risk" เลยตั้งแต่แรก (Sleep/Hold Person/Hypnotic Pattern/Banishment/Hold Monster/Dominate Monster/Reverse Gravity ฝั่ง Control และ Bless/Beacon/Haste/Holy Aura ฝั่ง Support ล้วน concentration แต่ Mass Suggestion/Aid/Death Ward/Freedom of Movement/ฮีลแทบทุกตัวไม่ต้องเลย) — ถาม user แล้วตัดสินใจ **เพิ่มมิติที่ 6 "Concentration Risk" เข้าทั้ง 2 ตารางจริงจัง** (ไม่ใช่แค่ note) พร้อมแก้ [Sub-dimension tie-break](#sub-dimension-tie-break--เกรดเดียวจากหลายมิติที่อาจไม่ตรงกัน-เพิ่ม-2026-08-14-รอบ-2-แก้ให้รองรับจำนวนมิติไม่เท่ากันรอบ-10) ให้ใช้เกณฑ์สัดส่วน (≥ครึ่งหนึ่งของ Modifier ปัดขึ้น) แทนเลข "≥2" ตายตัว เพื่อรองรับทั้ง axis 5-มิติและ 6-มิติพร้อมกัน — **นี่คือ methodology-level fix ตัวที่ 2 ของทั้งไฟล์ (ตัวแรกคือ peer-standard รอบ 7)** เพราะเพิ่มมิติใหม่จริง ไม่ใช่แค่แก้ถ้อยคำ

**🔧 Refinement pass รอบ 11 (2026-08-16)**: cross-check ตัวเลข/duration field ที่ยังไม่เคยตรวจตรงๆ (Reverse Gravity, Dominate Monster, Regenerate, Fire Bolt scaling, Dominate Monster full text, Power Word Stun/Kill full text, Mass Suggestion full text, Death Ward/Aid/Holy Aura/Heal full text) เทียบกับ `spells-xphb.json` โดยตรงทั้งหมด — **ตรงกับไฟล์จริงทุกจุด ไม่เจอบั๊กตัวเลข/กติกาเพิ่มเติมเลยรอบนี้** (สัญญาณว่าเนื้อหาเชิงข้อมูลลู่เข้าแล้วจริง หลังจากรอบ 8-9-10 ทยอยลดความรุนแรงของบั๊กที่เจอลงเรื่อยๆ) — เจอปัญหาประเภทใหม่แทน: **ลิงก์ anchor ภายในไฟล์เสีย** — changelog รอบ 2 (ย่อหน้าด้านบน) มีลิงก์ไปยัง [Sub-dimension tie-break](#sub-dimension-tie-break--เกรดเดียวจากหลายมิติที่อาจไม่ตรงกัน-เพิ่ม-2026-08-14-รอบ-2-แก้ให้รองรับจำนวนมิติไม่เท่ากันรอบ-10) ที่ยังชี้ไปยัง anchor slug แบบเก่า (ตอนหัวข้อยังชื่อ "...เกรดเดียวจาก 5 มิติ...") ทั้งที่หัวข้อจริงถูกเปลี่ยนชื่อไปแล้วตอนรอบ 10 (ตัด "5" ออก + เพิ่ม suffix รอบ 10) ทำให้ลิงก์นี้ชี้ไปยัง anchor ที่ไม่มีอยู่จริงอีกต่อไป — แก้ให้ชี้ไปยัง anchor ปัจจุบันแล้ว (ลิงก์เดียวกันที่ changelog รอบ 10 ใช้อยู่แล้วซึ่งถูกต้องอยู่แล้วไม่ต้องแก้)

**🔧 Refinement pass รอบ 12 (2026-08-16)**: ตรวจโครงสร้าง/ความสอดคล้องภายในไฟล์แทนตัวเลข spell — (a) นับจำนวนคอลัมน์ทุกแถวของทั้ง 7 ตารางเทียบกับ header เจอว่า**ตรงกันทุกแถวทุกตาราง ไม่มี column mismatch เลย**, (b) recompute เลขคณิต Effective HP (Survivability, สูตร fixed-HP: lv1=max+CON+3, ต่อไป+avg ปัดขึ้น+CON+3) ทุก 20 แถว และ DPR (Damage, ทุก track) ทุก 20 แถว เทียบกับสูตรที่ intro ประกาศไว้ — **ตรงกันทั้งหมด รวม rounding convention (round-half-up) ที่ใช้สม่ำเสมอทุกจุด**, (c) **เจอบั๊กจริง 1 จุด** — Versatility finding #3 (ตอนนั้นอยู่บรรทัดท้ายไฟล์) เขียนว่า "อ้างอิงย้อนไปยัง **5** axis ก่อนหน้า" ทั้งที่นิยามมิติ Cross-Axis Coverage ของ axis เดียวกัน (ในหัวข้อเดียวกัน ไม่กี่บรรทัดก่อนหน้า) ระบุไว้ชัดว่า "**6** axis อื่น" (Versatility เป็น axis ที่ 7/7 จึงมี 6 axis ก่อนหน้า ไม่ใช่ 5 — ตัวเลข 5 หลุดมาจากบริบทของ Utility finding #1 ที่ถูกต้อง เพราะ Utility เป็น axis ที่ 6/7 จึงมี 5 axis ก่อนหน้าจริง แต่ถูกก็อปมาใช้ผิดที่) — แก้เป็น 6 axis พร้อมระบุชื่อทั้ง 6 axis ให้ชัดเจนกันสับสนซ้ำ

**🔧 Refinement pass รอบ 13 (2026-08-16)**: ตรวจ automation เพิ่มอีก 3 มุม — (a) ตรวจว่าทุก "= lvN" reference ใน 7 ตาราง**ชี้ย้อนหลังเท่านั้น** (ไม่มี forward/self-reference) — ผ่านหมด, (b) ตรวจ "= lvN" chain ทุกคอลัมน์**ไม่มี indirect 2-hop reference หลงเหลือ** (บั๊กแบบที่แก้ไปแล้วตอนรอบ4) — ผ่านหมด ไม่เจอจุดใหม่, (c) ตรวจว่าคำว่า "N/A" ที่ยังเหลืออยู่ในไฟล์ทุกจุด**เป็นการอธิบายกฎ N/A→D เท่านั้น ไม่มีจุดไหนใช้ N/A เป็นเกรดจริงในตารางหลงเหลือ** — ผ่านหมด — **เจอจุดเดียวรอบนี้ เป็น cosmetic wording ไม่ใช่บั๊กเชิงข้อมูล**: ชื่อมิติในย่อหน้า intro กับชื่อคอลัมน์ในตารางจริงเขียนไม่ตรงกัน 2 จุด — Damage's "Burst-Peak" (intro) vs "Burst/Peak" (header จริง), Support's "Proactive vs Reactive"/"Frequency/Sustainability" (intro) vs "Proactive/Reactive"/"Frequency" (header จริง) — แก้ intro ให้ตรงกับ header (ตัวที่ใช้ให้คะแนนจริง) ทั้ง 2 axis แล้ว — **นี่คือรอบที่เบาที่สุดเท่าที่เคยเจอ (แค่ punctuation ไม่ใช่ความหมาย) ต่อเนื่องจากรอบ 11 (0 บั๊กข้อมูล) และรอบ 12 (1 บั๊ก off-by-one) — สัญญาณลู่เข้าชัดเจนขึ้นเรื่อยๆ**
...
ทุก checkpoint (1 axis × 1 เลเวล ของ 1 คลาส/subclass ในชั้น 1/2) ให้เกรดเดียวโดยเทียบมิติทั้งหมดของ axis นั้น (5 มิติสำหรับ 5 axis ปกติ, 6 มิติสำหรับ Control/Support หลังเพิ่ม Concentration Risk รอบ 10) กับแถว Anchor ที่ตรงกับ band ของเลเวลนั้น ใช้ตัวอักษรชุดเดียวกับระบบเดิม ([00-scorecard-methodology.md](00-scorecard-methodology.md)) เพื่อให้แปลงเป็น point ได้ทันทีถ้าวันหลังอยากรวมคะแนนข้าม axis (S=+3/A=+2/B=+1/C=0/D=-1 ตาม point formula ของ [[project_overall_tier_point_rework]]):
...
### Sub-dimension tie-break — เกรดเดียวจากหลายมิติที่อาจไม่ตรงกัน (เพิ่ม 2026-08-14 รอบ 2, แก้ให้รองรับจำนวนมิติไม่เท่ากันรอบ 10)

แต่ละ axis มีหลายมิติที่มักให้ผลไม่ตรงกัน (เช่น Wizard lv1 ใน Survivability: Effective HP=C แต่ Mitigation/Death Prevention=D ทั้งคู่) — ต้องมีกติกาแปลงเป็นเกรดเดียวที่ชัดเจน ไม่ใช่ให้ผู้ประเมินเดาเอาเอง **หมายเหตุรอบ 10**: Control/Support ตอนนี้มี 6 มิติ (เดิม 5) หลังเพิ่ม Concentration Risk ส่วน 5 axis ที่เหลือยังคง 5 มิติเท่าเดิม — กติกาด้านล่างเลยเขียนเป็นสัดส่วนแทนเลขตายตัว เพื่อรองรับทั้ง 2 กรณี:
...
2. **มิติที่เหลือเป็น "Modifier"** (4 มิติสำหรับ 5 axis ปกติ, 5 มิติสำหรับ Control/Support) — นับจำนวนมิติที่เหนือ peer standard (จะขยับเกรดตั้งต้น **ขึ้น 1 ขั้น** ถ้ามี**อย่างน้อยครึ่งหนึ่งของ Modifier ปัดขึ้น** เหนือมาตรฐาน — คือ ≥2/4 สำหรับ axis ปกติ, ≥3/5 สำหรับ Control/Support) กับจำนวนมิติที่ต่ำกว่า/ไม่มีกลไกเลย (ขยับเกรดตั้งต้น **ลง 1 ขั้น** ด้วยเกณฑ์สัดส่วนเดียวกัน) — ขยับได้สูงสุดครั้งละ 1 ขั้นเท่านั้น (กัน 1 มิติเด่นมากลากทั้งเกรดขึ้น S ทั้งที่ภาพรวมยังธรรมดา)
...
6 มิติที่ใช้ประเมิน: **Severity** / **Breadth** (กี่เป้าหมาย) / **Frequency** (กี่ครั้ง/วัน) / **Stickiness** (หลุดง่ายแค่ไหนฝั่ง**เป้าหมาย** — save ทุกเทิร์น / save ครั้งเดียวคงอยู่ / ไม่มี save เลย) / **LR-bypass** (ใช้ mechanism ที่ Legendary Resistance ต้านไม่ได้หรือเปล่า — สำคัญเฉพาะ lv15+ ที่เริ่มเจอบอสมี Legendary Resistance บ่อย) / **Concentration Risk** (เพิ่ม 2026-08-14 รอบ 10 — คาสเตอร์ต้อง concentration รักษาสเปลล์ไว้ไหม เป็นความเสี่ยงฝั่ง**คาสเตอร์** แยกจาก Stickiness ที่วัดฝั่งเป้าหมาย: ถ้าต้อง concentration แล้วคาสเตอร์โดนตี/soft-CC ก่อน = สเปลล์หลุดทันทีไม่ว่าเป้าจะ save พลาดกี่ครั้งก็ตาม — เพิ่มมิตินี้หลังพบว่า Mass Suggestion (lv11) ไม่ต้อง concentration เลยทั้งที่ CC spell ส่วนใหญ่ในตารางนี้ต้อง ทำให้ค่าจริงต่างจากที่ 5 มิติเดิมสื่อไว้)
...
| Lv | Spell อ้างอิง (slot) | Severity | Breadth | Frequency | Stickiness | LR-bypass | Concentration Risk |
|---|---|---|---|---|---|---|---|
| 1 | Sleep (slot1) — 2024 เปลี่ยนมาใช้ WIS save (ต่างจาก 2014 ที่ใช้ HP-pool) | **Tier 4** (Incapacitated ก่อน → Unconscious ถ้า save รอบสองพลาดด้วย — ยืนยันจาก spell text จริง ทั้งคู่เป็น Tier 4 ตามนิยาม Severity scale ด้านบน) | หลายตัวอ่อนๆ ใน 5ft radius | ~3-4/วัน | **ต่ำ** — save รอบถัดไปอีกที + หลุดถ้าโดนดาเมจ (ความอ่อนของสเปลล์นี้อยู่ที่ Stickiness ไม่ใช่ Severity — เดิมเคยลดเกรด Severity ลงมาเป็น Tier 2 เพื่อสะท้อนความอ่อน ซึ่งขัดกับนิยาม Tier scale เอง ทำให้ Severity/Stickiness ทับซ้อนกัน) | ไม่ (มี save) | **มี** — concentration 1 นาที (ยืนยันจาก `duration` field) |
| 2 | = lv1 (ไม่มี slot ใหม่) | = lv1 | = lv1 | = lv1 | = lv1 | = lv1 | = lv1 |
| 3 | Hold Person (slot2) | **Tier 4** (Paralyzed) | 1 (humanoid เท่านั้น) | ~3-4/วัน | **ต่ำ** — save ทุกจบเทิร์นเป้าหมาย หลุดเร็ว | ไม่ | **มี** — concentration 1 นาที |
| 4 | = lv3 (ไม่มี slot ใหม่) | = lv3 | = lv3 | = lv3 | = lv3 | = lv3 | = lv3 |
| 5 | Hypnotic Pattern (slot3) | Tier 4 (Charmed+Incap+Speed0) | **กว้าง** — ~10 ตัวใน cube 30ft | ~2-3/วัน | **สูง** — ไม่มี repeat save เลย หลุดแค่โดนดาเมจ/ถูกเขย่า | ไม่ (save แยกทีละตัว) | **มี** — concentration 1 นาที |
| 6 | = lv5 (ไม่มี slot ใหม่) | = lv5 | = lv5 | = lv5 | = lv5 | = lv5 | = lv5 |
| 7 | Banishment (slot4) | Tier 4 (Incap เต็ม duration — **+ ถ้าเป้าเป็น Aberration/Celestial/Elemental/Fey/Fiend และ concentration อยู่ครบ 1 นาทีเต็ม เป้าจะไม่กลับมาเลย ถูกส่งไปยัง plane ที่เกี่ยวข้องถาวรแทน** — เท่ากับ removal ถาวรสำหรับ creature type เหล่านี้ ไม่ใช่แค่ "หายไปชั่วคราวจนจบ duration" แบบที่ target อื่นเจอ ยืนยันจาก spell text จริง เดิมตารางเคยละจุดนี้ไว้) | 1-2 | ~2/วัน | **สูง** — ไม่มี repeat save จนกว่า concentration จะขาด | ไม่ | **มี** — concentration 1 นาที (แต่ถ้าครบ 1 นาทีเต็มกับ creature type ที่ถูกล็อกถาวรแล้ว concentration หลังจากนั้นไม่มีผลอะไรอีก เพราะผลถาวรไปแล้ว) |
| 8 | = lv7 (ไม่มี slot ใหม่) | = lv7 | = lv7 | = lv7 | = lv7 | = lv7 | = lv7 |
| 9 | Hold Monster (slot5) | Tier 4 (Paralyzed, ทุกชนิด) | 1 | ~2/วัน | **ต่ำ** — save ทุกเทิร์นเหมือน Hold Person (⚠️ slot สูงขึ้นไม่ได้แปลว่า sticky ขึ้นเสมอ) | ไม่ | **มี** — concentration 1 นาที |
| 10 | = lv9 (ไม่มี slot ใหม่) | = lv9 | = lv9 | = lv9 | = lv9 | = lv9 | = lv9 |
| 11 | Mass Suggestion (slot6) | Tier 3 (Charmed) | **กว้างมาก** — สูงสุด 12 ตัว | ~1-2/วัน | **สูงสุดในตาราง — ไม่ใช่แค่ "สูง"** — หลุดแค่โดนดาเมจเท่านั้น + duration จริง 24 ชม. (ยืนยันจาก `duration` field) | ไม่ | **⭐ ไม่มีเลย — จุดเด่นที่สุดของแกนนี้** — ไม่ใช่ concentration (ต่างจาก Sleep/Hold Person/Hypnotic Pattern/Banishment/Hold Monster/Dominate Monster ที่ล้วนต้อง concentration ทั้งหมด) แปลว่าคาสเตอร์ cast แล้วไปทำอย่างอื่น/cast concentration spell อื่นซ้อนได้ทันที ไม่มีความเสี่ยงฝั่งคาสเตอร์เลย — ข้อได้เปรียบเชิงโครงสร้างที่ใหญ่ที่สุดจุดหนึ่งของ Control axis |
| 12 | = lv11 (ไม่มี slot ใหม่) | = lv11 | = lv11 | = lv11 | = lv11 | = lv11 | = lv11 |
| 13 | *(ไม่มี condition-lock เด่นที่ slot7 — control เปลี่ยนรูปเป็น battlefield control เช่น Reverse Gravity)* | ผสม | กว้าง (AoE) | ~1/วัน | แล้วแต่ spell | ไม่ | **มี** — Reverse Gravity เป็น concentration 1 นาที (ตัวแทนหลักของ slot7 ในตารางนี้) |
| 14 | = lv13 (ไม่มี slot ใหม่) | = lv13 | = lv13 | = lv13 | = lv13 | = lv13 | = lv13 |
| 15 | **Power Word Stun** (slot8) — ทางเลือก: *Dominate Monster* (Tier 3 Charmed เต็มรูปแบบ, stickiness **สูง**-save แค่ตอนโดนดาเมจ, ไม่ LR-bypass) | Tier 4 (Stunned ถ้า HP≤150 — **fallback ถ้า HP>150: Speed=0 จนถึงต้นเทิร์นถัดไปของผู้คาสต์เท่านั้น** ไม่ใช่ "ไม่เกิดอะไรเลย" แบบที่เคยละไว้ ยืนยันจาก spell text) | 1 | ~1/วัน | ต่ำ (หลุดด้วย CON save ทุกเทิร์นหลังติด) | **✅ ใช่ — ไม่มี save ตอนติดเลย** | **Power Word Stun: ไม่มี** (instant — Stunned เป็น condition ที่ยึดอยู่เอง ไม่ผูกกับคาสเตอร์อีกต่อไปหลัง cast) แต่ **Dominate Monster (ทางเลือก): มี** — concentration 1 ชม. (trade-off ชัดเจนอีกชั้น: Power Word Stun ไม่ concentration แต่ LR ต้านไม่ได้กับ HP-gate, Dominate Monster concentration แต่ sticky กว่าและ LR ต้านได้) |
| 16 | = lv15 (ไม่มี slot ใหม่) | = lv15 | = lv15 | = lv15 | = lv15 | = lv15 | = lv15 |
| 17 | **Power Word Kill** (slot9) | สูงสุด (ตายทันทีถ้า HP≤100 — **fallback ถ้า HP>100: 12d12 Psychic damage (~78) แทน** ไม่ใช่ "ไม่เกิดอะไรเลย" แบบที่เคยละไว้ ยืนยันจาก spell text — สำคัญเพราะบอสระดับ lv17+ มักเกิน 100 HP จึงมักเจอ fallback นี้บ่อยกว่าเอฟเฟกต์ตายทันที) | 1 | ~1/วัน | ไม่มีความหมายให้วัดเมื่อเป้าตาย — เป้าตายจบเกมทันที ไม่เหลืออะไรให้ "หลุด" อีก (คนละความหมายกับ "ไม่มีกลไก" ที่ให้ D ตาม Grading Scale — นี่คือผลลัพธ์ที่ดีที่สุดเท่าที่เป็นไปได้ ไม่ใช่การขาดกลไก); กรณี fallback (HP>100) ดาเมจ 12d12 ก็ไม่มี save ต้านเช่นกัน | **✅ ใช่ — ไม่มี save เลย ทั้ง 2 กรณี** | **ไม่มี** (instant — ตายหรือโดนดาเมจทันทีตอน cast ไม่มีอะไรให้ concentration รักษาต่อ) |
| 18 | = lv17 (ไม่มี slot ใหม่ — slot9 คือเพดานสูงสุด ไม่มี slot10) | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 |
| 19 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 |
| 20 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 |
...
5. ⚠️ **เพิ่มมิติที่ 6 — Concentration Risk (รอบ 10)**: เดิม 5 มิติ (Severity/Breadth/Frequency/Stickiness/LR-bypass) ไม่มีมิติไหนวัด **"caster concentration risk"** เลย — เช็คไฟล์จริงพบว่า Sleep/Hold Person/Hypnotic Pattern/Banishment/Hold Monster/Dominate Monster/Reverse Gravity ล้วนเป็น concentration spell (caster โดนตีแล้วเสีย concentration = สเปลล์หลุดทันทีไม่ว่าเป้าจะ save พลาดแค่ไหน) ยกเว้น **Mass Suggestion (lv11, ไม่ concentration + 24ชม. — จุดเด่นสุดของแกนนี้)** และ Power Word Stun/Kill (instant, ไม่มี ongoing concentration เพราะ effect ผูกกับ target เองไม่ใช่คาสเตอร์) — user ยืนยันให้เพิ่มเป็นมิติที่ 6 จริงจัง (ไม่ใช่แค่ note ท้ายตาราง) เพิ่มคอลัมน์ Concentration Risk ในตารางแล้วครบทุกแถว

---

## Support Anchor

Support มีสองสาย mixed กัน ที่มูลค่าจริงต่างกันมาก: **ฮีล** (แก้หลังเกิดเรื่อง) กับ **บัฟ** (กันก่อนเกิดเรื่อง) — 6 มิติที่ใช้ประเมิน: **Magnitude** (ค่าที่ได้ต่อครั้ง) / **Breadth** (เดี่ยว vs ทั้งปาร์ตี้) / **Action Economy** (bonus action vs full action vs free ผ่าน aura) / **Proactive/Reactive** (บัฟก่อนเกิดเรื่องมักคุ้มกว่าฮีลหลังเกิดเรื่อง เพราะฮีลแค่ลบล้างดาเมจที่ทำไปแล้ว ไม่เพิ่มมูลค่าใหม่ให้ encounter) / **Frequency** (จำกัดครั้ง/วัน vs aura ต่อเนื่อง vs downtime-only — sustainability ในความหมายเดียวกัน) / **Concentration Risk** (เพิ่ม 2026-08-14 รอบ 10 — คู่กับ Control axis: ฮีลเกือบทั้งหมด instant ไม่ต้อง concentration แต่บัฟส่วนใหญ่ต้อง concentration ยกเว้น Aid/Death Ward/Freedom of Movement ซึ่งเป็นจุดได้เปรียบเชิงโครงสร้างของบัฟกลุ่มนั้น)
...
| Lv | Spell อ้างอิง (slot) | Magnitude | Breadth | Action Economy | Proactive/Reactive | Frequency | Concentration Risk |
|---|---|---|---|---|---|---|---|
| 1 | Bless / Cure Wounds / **Healing Word** (slot1) | ฮีล **Cure Wounds** 2d8+3(**~12**, full action, touch) vs **Healing Word** 2d4+3(**~8**, bonus action, 60ft) — ยืนยันจาก `spells-xphb.json`: 2024 ทั้งคู่อัปเกรดจาก 2014 (Cure Wounds 1d8→2d8, Healing Word 1d4→2d4) แต่ **เต๋าคนละขนาดกันจริง ไม่ใช่ ~12 เท่ากันแบบที่เคยเขียนรวมไว้** — Healing Word เบากว่าจริง แลกกับความสะดวก bonus action+ระยะ ไม่ใช่ magnitude เท่ากัน — เดี่ยวทั้งคู่ / Bless +1d4 atk/save 3 คน | สูงสุด 3 (Bless) / เดี่ยว (ฮีล) | **ผสม** — Healing Word เป็น **bonus action** (ฮีลได้โดยยังโจมตีได้ปกติ) ส่วน Cure Wounds/Bless เป็น full action | ทั้งสองสายมีตั้งแต่ lv1 | ~3-4/วัน | **ผสม** — Cure Wounds/Healing Word instant ไม่ concentration แต่ **Bless ต้อง concentration 1 นาที** (ยืนยันจาก `duration` field) |
| 2 | = lv1 (ไม่มี slot ใหม่) | = lv1 | = lv1 | = lv1 | = lv1 | = lv1 | = lv1 |
| 3 | Aid (slot2) | +5 max/current HP ×3 คน — อยู่ทั้งวัน ไม่ต้อง cast ซ้ำ | 3 คน | full action แต่ cast ครั้งเดียวคุ้มทั้งวัน | **Proactive** | **1 ครั้ง/วันก็พอ** — ผลอยู่ 8 ชม. เต็มรอบผจญภัย (เดิมเขียน "~3-4/วัน" ปนกับจำนวน slot ที่มี ทำให้เข้าใจผิดว่าต้อง cast ซ้ำหลายครั้ง — แก้ให้ตรงกับนิยาม Frequency จริง คือ "ต้อง cast กี่ครั้ง" ไม่ใช่ "มี slot กี่อัน") | **ไม่มี** — instant ไม่ผูก concentration เลย (แถมยังอยู่ 8ชม.) เป็นข้อได้เปรียบซ้อนกับ Frequency |
| 4 | = lv3 (ไม่มี slot ใหม่) | = lv3 | = lv3 | = lv3 | = lv3 | = lv3 | = lv3 |
| 5 | Beacon of Hope / Haste (slot3) | **สูง** — Haste ให้ action เพิ่มทั้งตัว: **2024 อนุญาต Attack ได้ 1 ครั้งด้วย** (Attack[one attack only]/Dash/Disengage/Hide/Utilize — ยืนยันจาก `spells-xphb.json` entry text ตรงๆ, แก้จากที่เคยเขียนผิดว่า "ยกเว้น attack" ซึ่งเป็นกติกา 2014 เก่าที่ 2024 เปลี่ยนไปแล้ว) แต่มีดาบสองคม — เมื่อสเปลล์หมดฤทธิ์เป้าหมายติด Incapacitated+Speed0 จนจบเทิร์นถัดไป (lethargy), **แก้คำอธิบาย Beacon of Hope** — spell text จริงไม่ได้ "กัน unconscious" ตรงๆ (เดิมเขียนเกินจริง) ที่จริงคือ advantage WIS save + advantage **Death Saving Throw** + ฮีลที่ได้รับนับเป็นค่าสูงสุดเสมอ (max healing) ตลอด duration 1 นาที (concentration) — ผลคือช่วยให้ "รอดจาก death save และฟื้นตัวได้เต็มที่กว่า" ไม่ใช่ "ไม่มีวันสลบเลย" | เดี่ยว (ทั้งคู่ single target) | full action, ผลอยู่ 1 นาที (concentration ทั้งคู่ — Haste/Beacon durationสั้นเท่าคอมแบตทั่วไป ไม่ใช่ long-duration buff) | **Proactive** | ~2-3/วัน | **มี ทั้งคู่** — concentration 1 นาที |
| 6 | = lv5 (ไม่มี slot ใหม่) | = lv5 | = lv5 | = lv5 | = lv5 | = lv5 | = lv5 |
| 7 | Death Ward / Freedom of Movement (slot4) | **สูงมาก — จุดเปลี่ยน** — Death Ward กัน HP=0 ครั้งนึงฟรี (cheat-death เต็มรูปแบบ) **+ กัน instant-death effect ที่ไม่ใช่ดาเมจด้วย** (เช่น disintegrate ตอน HP เหลือ0, sphere of annihilation — ยืนยันเพิ่มจาก entry ที่ 2 ของ spell text, เดิมเขียนไว้แค่ effect แรก), Freedom of Movement กัน paralyze/restrain/grapple ทั้งดื้อตลอด duration | เดี่ยว | full action, ผลอยู่ 8ชม.(Death Ward)/1ชม.(Freedom of Movement) — **ทั้งคู่ไม่ใช่ concentration** (ต่างจาก Beacon/Haste/Holy Aura ในตารางนี้ที่ต้อง concentration ทั้งหมด) แปลว่า cast ล่วงหน้าแล้วยัง concentration สเปลล์อื่นพร้อมกันได้ ยืนยันจาก duration field ในไฟล์จริงไม่มี flag concentration — จุดนี้เคยไม่ถูกพูดถึงเลย เป็นข้อได้เปรียบเชิง action economy ที่แท้จริง | **Proactive ล้วน** — Magnitude แซงฮีลไปไกล | ~2/วัน | **⭐ ไม่มี ทั้งคู่** — จุดเด่นสุดของ Support axis (คู่กับ Mass Suggestion ใน Control): บัฟระดับสูงมากแต่ไม่ผูก concentration เลย cast ซ้อนกับ Bless/Haste/Beacon/Holy Aura ได้พร้อมกัน |
| 8 | = lv7 (ไม่มี slot ใหม่) | = lv7 | = lv7 | = lv7 | = lv7 | = lv7 | = lv7 |
| 9 | Mass Cure Wounds / Greater Restoration (slot5) | 5d8+3 เฉลี่ย 22.5+3=25.5 → **~26**/คน ×6 คน (**~156** รวม — แก้เลขปัดจาก ~27/~162 เดิมที่ปัดขึ้นเกินจริง 1.5) หรือแก้ debuff หนัก (curse/stat drain/exhaustion) | กว้าง (6 คน) | full action | **Reactive** กลับมาโดดเด่นด้วย breadth | ~2/วัน | **ไม่มี ทั้งคู่** — instant |
| 10 | = lv9 (ไม่มี slot ใหม่) | = lv9 | = lv9 | = lv9 | = lv9 | = lv9 | = lv9 |
| 11 | Heal (slot6) | **70 HP คงที่** + แก้ blind/deaf/poison | เดี่ยว | full action | Reactive | ~1-2/วัน | **ไม่มี** — instant |
| 12 | = lv11 (ไม่มี slot ใหม่) | = lv11 | = lv11 | = lv11 | = lv11 | = lv11 | = lv11 |
| 13 | Regenerate (slot7) | 4d8+15 + ฟื้น 1HP/เทิร์นต่อเนื่อง | เดี่ยว | **downtime-only** (cast 1 นาที ใช้กลางคอมแบตไม่ได้) | Reactive | ~1/วัน (นอกคอมแบต) | **ไม่มี** — ไม่ผูก concentration (แต่ downtime-only อยู่แล้วจากข้อจำกัด cast time ไม่ใช่จาก concentration) |
| 14 | = lv13 (ไม่มี slot ใหม่) | = lv13 | = lv13 | = lv13 | = lv13 | = lv13 | = lv13 |
| 15 | **Holy Aura** (slot8) | advantage ทุก save + disadvantage ศัตรูโจมตี + blind fiend/undead ที่ตี | **ทั้งปาร์ตี้** (ไม่จำกัดจำนวน) | full action, ผลอยู่ 1 นาที (concentration — เท่า Beacon/Haste ไม่ใช่ long-duration ต่างจาก Death Ward/Freedom of Movement ที่ lv7 ซึ่งไม่ต้อง concentration) | **Proactive สูงสุด** | ~1/วัน | **มี** — concentration 1 นาที (บัฟที่แรงที่สุดในตารางกลับเป็นตัวที่เปราะที่สุดฝั่งคาสเตอร์ด้วย) |
| 16 | = lv15 (ไม่มี slot ใหม่) | = lv15 | = lv15 | = lv15 | = lv15 | = lv15 | = lv15 |
| 17 | Mass Heal / Power Word Heal (slot9) | สูงสุด 700 HP แบ่งได้ทุกคน หรือฮีลเต็ม 100% ทันทีแบบไม่มีเงื่อนไข | เลือกได้ (กว้างสุด vs การันตีเดี่ยว) | full action | Reactive | ~1/วัน | **ไม่มี ทั้งคู่** — instant |
| 18 | = lv17 (ไม่มี slot ใหม่ — slot9 คือเพดานสูงสุด) | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 |
| 19 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 |
| 20 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 | = lv17 |
...
3. **Concentration Risk แบ่งแยกชัดตามประเภท (รอบ 10)**: ฮีลแทบทุกตัว (Cure Wounds/Healing Word/Mass Cure Wounds/Greater Restoration/Heal/Mass Heal/Power Word Heal) เป็น **instant ไม่ผูก concentration เลยสักตัว** — สมเหตุสมผลเพราะฮีลต้องใช้ทันทีตอนวิกฤต จะให้ concentration ไม่ได้ ส่วนบัฟแบ่งเป็น 2 กลุ่มชัดเจน: **กลุ่มไม่ concentration** (Aid, Death Ward, Freedom of Movement) ได้เปรียบเชิงโครงสร้างเพราะ cast ซ้อนกับบัฟ concentration อื่นได้ vs **กลุ่ม concentration** (Bless, Beacon of Hope, Haste, Holy Aura) เสี่ยงหลุดถ้าคาสเตอร์โดนตี — คู่ขนานกับ Control axis ที่ Mass Suggestion โดดเด่นเพราะไม่ concentration เหมือนกัน
...
| 3 | Knock (slot2) | แก้ปัญหาเฉพาะจุดเบ็ดเสร็จ (ล็อก/โซ่ใดๆ เปิด/พังทันที ไม่ต้องทอย check — **ข้อแม้**: ถ้าของมีหลายล็อกพร้อมกัน สเปลล์เปิดได้แค่ 1 ล็อกต่อ 1 cast + เกิดเสียงดังได้ยินไกล 300ft ทุกครั้งที่ cast ทั้ง 2 จุดนี้เคยละไว้ในตาราง ยืนยันจาก spell text จริง) | แคบมาก (เฉพาะของถูกล็อก) | auto-success (ต่อ 1 ล็อก) | เสีย slot จริง จำกัดครั้ง/วัน | = lv2 |
...
| 15 | Mind Blank (slot8) | ป้องกัน divination/mind-reading/charm ทั้งหมด 24ชม. (**+ immune ดาเมจ Psychic ด้วย** — ยืนยันจาก spell text: "Immunity to Psychic damage and the Charmed condition" เดิมเขียนถึงแค่ครึ่งข้อมูล/charm ไม่ได้พูดถึง Psychic immunity ซึ่งจริงๆ เป็นมูลค่าฝั่ง Mitigation ไม่ใช่แค่ข้อมูล แต่ยังนับอยู่ใน Utility axis เพราะเป็น milestone เดียวกัน ไม่แยกไปนับซ้ำใน Survivability) — utility เชิงป้องกันข้อมูล ไม่ใช่แก้ปัญหาเดินทาง/หาข้อมูล | แคบ (จุดประสงค์เดียว) แต่สมบูรณ์แบบ (กันได้แม้ effect ระดับ wish) | auto-success (ตัวเองยินยอมรับผล) | slot | = lv9 (คงที่) |
...
3. **Versatility เป็น axis เดียวที่อ้างอิงย้อนไปยัง 6 axis ก่อนหน้าโดยตรง** (Damage/Control/Support/Survivability/Action Economy/Utility — ตรงกับนิยาม "6 axis อื่น" ของมิติ Cross-Axis Coverage ด้านบน) ผ่านมิติ Cross-Axis Coverage — เหตุผลที่ต้องทำเป็น axis สุดท้ายตามลำดับที่วางแผนไว้ เพราะต้องมี anchor อื่นเสร็จก่อนถึงจะนับ coverage ได้จริง
...
2. เริ่มชั้น 1 (30 Class Baseline level-indexed) — ยังไม่เริ่ม รอ user ยืนยันก่อนเริ่มงานจริง เพราะเป็น scope ใหญ่ (30 คลาส × 20 เลเวล × 7 axis) — progress tracker สร้างไว้แล้วที่ [00-level-progress.md](00-level-progress.md) (2026-08-16, ทุกช่อง `[ ]` ยังไม่ประเมิน)
