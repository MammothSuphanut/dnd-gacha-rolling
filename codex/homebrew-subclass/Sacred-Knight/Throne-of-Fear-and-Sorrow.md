# Throne of Fear and Sorrow

*Known in the ranks as the Knight of Destruction and Forgiveness — a title that carries its own contradiction. A knight of this Throne is devastating without restraint, and merciful without hesitation, and never sees those two truths as being in conflict. They break their enemies first, and only afterward decide whether to spare them.*

## Sacred Throne Features

### 3rd Level: Sorrow's Toll

*Power drawn from the body itself, paid for in blood rather than rest.*

Whenever you take the bonus action to convert a spell slot into Fervor, you can also pay hit points equal to 3 times your Wisdom modifier to gain bonus Fervor equal to your Wisdom modifier. You can use this a number of times equal to your Proficiency Bonus per long rest, preventing you from bleeding yourself dry without limit.

### 6th Level: Moment of Dread

*Fear, at its sharpest, kills as surely as steel.*

When you make an attack using Faith Halo, if the resulting attack roll is at least the target's AC + 15 + the target's Proficiency Bonus, that attack counts as a critical hit. You can trigger this only once per turn, even if you make several attacks that turn — each individual attack still requires you to pay Faith Halo's cost separately, as normal, but only one of them can become a critical hit this way, preventing this from combining with rapid multi-attacks for repeated automatic crits.

### 10th Level: Mercy's Embrace

*Even ruin can be tempered, if you choose to spend less of yourself.*

Sorrow's Toll becomes more efficient: the hit point cost drops from 3 times your Wisdom modifier to 2 times your Wisdom modifier, for the same Fervor gained. This shares the same per-long-rest use cap as Sorrow's Toll — it doesn't grant additional uses.

### 14th Level: Wrath of Ruin

*When the blow finally lands, let it land absolutely.*

When you score a critical hit with an attack using Faith Halo, that attack deals triple damage instead of double. This is still bound by Moment of Dread's once-per-turn limit.

### 17th Level: Throne of Fear and Sorrow

*To become an engine of ruin, you must first accept what it costs you.*

You can activate this Throne by spending spell slots with a combined level of 10 (calculated the same way as converting Fervor, but spent to activate the Throne rather than banked as Fervor). Once activated, the Throne's effect lasts for 1 minute.

While active, at the start of each of your turns — no longer tied to your bonus action — you are forced to take 20 damage immediately (real damage to your current hit points, not a reduction to your maximum) in exchange for 10 Fervor automatically, along with the effects of Dash and Disengage for free that turn.

This cost is mandatory, not optional. If a turn passes in which you don't actually take that 20 damage — because of an immunity, a ward, or any other effect that prevents it — the Throne shuts off immediately, before its full duration ends. You can also choose to end it early yourself at any time if you don't want to keep paying the price. Running it for the full 10 rounds costs up to 200 hit points in total — a steep, intentional risk that matches this Throne's theme of devastation without restraint.
