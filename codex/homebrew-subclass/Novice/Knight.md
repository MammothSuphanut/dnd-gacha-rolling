# Knight

*The oldest and most straightforward of the Novice's Job Paths: pick up a heavy weapon, plant your feet, and refuse to fall. A Knight doesn't out-think a fight — they out-last it, trading blows toe-to-toe until the enemy breaks first. By the time a Knight reaches the top of this path as a Dragon Knight, their body has stopped being just flesh and steel and started being something closer to the dragons whose fortitude they've come to share.*

## Knight Traits

**Primary Ability.** Strength.

Choosing the Knight Job Path at 3rd level doesn't grant weapon or armor proficiencies by itself — those come from **Battle Will**, below, the moment you choose this path.

## Job Path Features

As a Knight, you gain the following features at each of your Job Changes.

### 3rd Level: Second Class — Knight

*You stop being someone who merely swings a weapon and start being someone the front line can be built around.*

**Battle Will.** You gain proficiency with Martial melee weapons, and you gain training with Medium armor, Heavy armor, and Shields.

**Steel Heart.** Your body toughens as you throw yourself into every fight. Your maximum hit points increase by 3 for every level you have in this class. This bonus is recalculated whenever your Novice level changes — it's +9 at 3rd level, growing all the way to +60 by 20th level.

### 7th Level: Transcendent Second Class — Lord Knight

*A Lord Knight's fury is a resource in its own right — one they can call on exactly when it matters most.*

**Berserk.** As an action, once per Long Rest, you give yourself over to battle fury: you instantly regain all of your lost hit points, but your Mana immediately drops to 0.

### 11th Level: Third Class — Rune Knight

*Runic strength doesn't just protect a Rune Knight — it teaches their body to recover from being caught off guard.*

**Rune Shield.** Whenever you're hit by an attack, you gain a +1 bonus to AC, lasting until the start of your next turn. This bonus stacks with itself, up to a maximum of +3.

**Rune Blessing.** Whenever you fail a saving throw, the next saving throw you make within 1 minute after that failure is made with advantage.

### 15th Level: Fourth Class — Dragon Knight

*The final Job Change. A Dragon Knight's body has stopped behaving like an ordinary mortal's — their strikes cut clean through what should stop them, and their skin shrugs off what should break it.*

**Dragonic Weapon.** Your Strike ignores resistance to the damage type it deals.

**Dragonic Defense.** You gain resistance to Bludgeoning, Piercing, and Slashing damage from nonmagical attacks.

## Knight Job Skills

The 9 Job Skills below are the Knight Job Path's pool. As described under *Job Skills* in the Novice class document, you choose **2** of these at 3rd level and **1** more each at 6th, 10th, 14th, and 18th level (6 total), each starting at your current Rank — and every one you know advances in Rank alongside your Class Die (see *Job Skills* in the Novice class document).

### Focus Strike
*Job Skill*

**Range:** Self
**Mana Cost:** 1

Use your Strike, with a bonus to the attack roll equal to 1 roll of your Class Die.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Aura Weapon
*Job Skill*

**Range:** Self
**Mana Cost:** 2
**Duration:** 1 minute

For the duration, your Strike deals extra Fire damage equal to 1 roll of your Class Die.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Auto Counter
*Job Skill*

**Range:** Self
**Mana Cost:** 1
**Duration:** 1 minute

Each time you're hit by a melee attack while this is active, you immediately use your Strike against the attacker. If it hits, the target takes extra damage equal to 1 roll of your Class Die, and this effect ends. If it misses, this effect remains active — keep making a counterattack each time you're hit until one lands or the duration expires.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Mark
*Job Skill*

**Range:** 60 feet
**Mana Cost:** 1
**Duration:** 1 minute

You mark one creature you can see within range. The first time each turn the marked creature is hit by an attack, from any source, it takes extra Fire damage equal to 1 roll of your Class Die (no more than once per turn).

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Joint Beat
*Job Skill*

**Range:** Self
**Mana Cost:** 1

Use your Strike. If it hits, the target takes extra damage equal to 1 roll of your Class Die, and it must make a Constitution saving throw against your Job Skill save DC. On a failure, until the start of your next turn, the target takes damage equal to 2 rolls of your Class Die if it moves more than 1 foot on its turn.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Phantom Strike
*Job Skill*

**Range:** Self
**Mana Cost:** 1

Use your Strike. If it hits, the target takes extra Fire damage equal to 3 rolls of your Class Die. If it misses, you don't spend the Mana Cost.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Increase HP Recovery
*Job Skill*

**Range:** Self
**Mana Cost:** 1
**Duration:** 1 minute

You gain hit points equal to half the number of sides on your Class Die, rounded down (**3** at d6, **4** at d8, **5** at d10, **6** at d12), every 6 seconds or at the start of each of your turns.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Self-Ignite
*Job Skill*

**Range:** Self
**Mana Cost:** 2
**Duration:** 1 minute (or until you end it)

You gain resistance to Fire damage, then ignite yourself, burning until you end it. At the end of each of your turns while active, choose any number of creatures within 5 feet of you. Each creature you choose must make a Constitution saving throw against your Job Skill save DC, taking Fire damage equal to 2 rolls of your Class Die on a failure, or half as much on a success. You can end this effect at any time as a Free Action.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.

### Guard
*Job Skill*

**Range:** Self
**Mana Cost:** 1

You gain temporary hit points equal to 2 rolls of your Class Die + your Novice level.

*Job Skill Advancement.* Scales automatically with your Class Die — see *Job Skills* in the Novice class document.
